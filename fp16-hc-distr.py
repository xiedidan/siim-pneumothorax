...
TRAIN = os.path.join(data_path, 'ori_train')
TEST = os.path.join(data_path, 'ori_test')
MASKS = os.path.join(data_path, 'ori_mask')
...
stats = {
    1024: ([0.491,0.49,0.49],[0.249,0.248,0.248])
}
...
class DynamicUnet_Hcolumns(SequentialEx):
    ...
    def __init__(self, encoder:nn.Module, n_classes:int, blur:bool=False, blur_final=True, 
        ...
        # convert conv1 in resnet to 4 channel
        encoder[0] = nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3)
        print(encoder)
...
for fold in range(args.fold):
    ...
    learn = learn.to_fp16().to_distributed(args.local_rank)
    learn.clip_grad(1.0)
